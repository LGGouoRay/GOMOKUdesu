...
void Game::initLobbyUI() {
    ...
    if (m_playMode == PlayMode::NetworkHost) {
        ...
        auto btnStartExp = std::make_unique<Button>(m_renderer.getFont(), std::string(reinterpret_cast<const char*>(u8"實驗模式")), sf::Vector2f(cx + 20.f, startY), size);
...
void Game::update(float dt) {
    ...
    while (it != m_emojiMessages.end()) {
        it->life -= dt;
        it->y -= 15.f * dt; // 改為非常緩慢地上升，因為有新訊息時會強制推高
        if (it->life <= 0.f) {
